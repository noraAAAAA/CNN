...
class Dataset:
    ...
    #加载数据集并交叉验证
    def load(self,img_rows = 96, img_cols = 96,img_channels = 3, nb_classes = 40):
        #加载数据集到内存
...
class Model:
    def __init__(self):
        self.model = None
    #build model
    def build_model(self, images, nb_classes = 40):
        ...
        # 训练模型
    def train(self, dataset, batch_size= 64, epochs = 200,data_augmentation = True):
        ...
        #                          samples_per_epoch=dataset.train_images.shape[0],epochs=epochs,
...
if __name__ == '__main__':
    ...
    print("Train-------------------------")
    model.train(dataset)
    model.save_model(file_path='./face.Model')
    model.load_model(file_path='./face.Model')
    print("Test--------------------------")
